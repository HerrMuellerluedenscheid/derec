...
if __name__ ==  "__main__":
    ...
    if test_type == 'castor':
        ...
        depths=num.arange(800., 9000.,200.)
    ...
    z, p, k = butter(2, [0.01*num.pi*2, 6.0*num.pi*2.],
                       'band',
                       analog=True,
                       output='zpk')
    ...
    test_case_setup.percentage_of_shift= 10.
    ...
    test_case.pre_highpass = (2, 0.02)
    test_case.individual_scaling = False
    test_case.scaling_factors = num.arange(0.1, 8., 0.1)
    ...
    plt.gcf().savefig('castor_misfits_indvl%s.pdf'%(test_case.individual_scaling),
                      dpi=300,
                      pad_inches=0.01,
                      bbox_inches='tight')
    plt.figure()
    ob.stack_plot()
    f = plt.gcf()
    f.savefig('castor_stackplot.pdf', dpi=300)
    plt.figure()
    ob.stack_plot(scaling=test_case.scaling, force_update=True)
    f = plt.gcf()
    if not test_case.individual_scaling:
        f.savefig('castor_stackplot_scaled.pdf', dpi=300)
    #ob.plot_misfits()
    plt.show()
