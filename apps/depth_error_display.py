...
f.close()
fn = 'results/depth_error_7.0.yaml'
f = open(fn, 'r')
data2 = load_string(f.read())
f.close()

lines1 = TestCase.lines_dict(data1.candidates)
lines2 = TestCase.lines_dict(data1.processed_candidates)
lines3 = TestCase.lines_dict(data1.processed_references)
...
for s1,t1,l1 in TestCase.iter_dict(lines1):
    for s2, t2, l2 in TestCase.iter_dict(lines2):
        for s3, t3, l3 in TestCase.iter_dict(lines3):
            ...
            if s1.__dict__==s2.__dict__==s3.__dict__ and t1.__dict__==t2.__dict__==t3.__dict__:
                print dir(l1)
                l1.set_linestyle('--')
                #l3.set_linestyle('')
                lines_match = [l1, l2, l3]
...
# contour plot risetime/depth/misfit
