...
if __name__ ==  "__main__":
    number_inversions = 100

    for numinv in range(number_inversions):
        use_markers = True  
        use_cake = True
        test_type  = 'doctar'
        invert = True
        ...
        if test_type == 'castor':
            ...
            stations = model.load_stations(data_base_dir+'/stations_sel.txt')
            ...
            depths=num.arange(1000., 9000.,1000.)
            if use_markers:
        ...
        z, p, k = butter(2, [0.70*num.pi*2, 5.0*num.pi*2.],
                           'band',
                           analog=True,
                           output='zpk')
        ...
        test_case_setup.static_length = 2.0
        test_case.individual_scaling = True
        test_case.scaling_factors = num.arange(0.1, 2., 0.1)
        test_case.blacklist = (('Y7','L001','', 'N'),
                              ('Y7','L004','','N'),
                              ('Y7','L005','','N'),
                              ('ES','EMOS','','N'))

        test_case.set_raw_references(reference_seismograms)
        du.randomize_DCSource(ref_source, inplace=True)
        refsourcestr = '0.0 %s %s %s\n'%(ref_source.strike, ref_source.dip,\
                                                       ref_source.rake)
        ...
        if invert:
            ...
            inv.tests_per_step = 20
            inv.set_focus([360., 90., 45., 15])
            inv.run(1, verbose=True)
            results = inv.get_results_unformatted()
            elapsed = time.time()-tstart
            f = open('inversion_%s.txt'%numinv, 'w')
        ...
        if not invert:
            ...
            plt.gcf().savefig('doctar_misfits.pdf', 
                      format='pdf',
                      dpi=300, 
                      bbox_inches='tight',
                      pad_inches=0.01)
            ...
            ob.stack_plot()
            f = plt.gcf()
            f.savefig('doctar_stackplot.pdf', 
                      format='pdf',
                      dpi=300,
                      bbox_inches='tight',
                      pad_inches=0.01)
            ...
            f = plt.gcf()
            f.savefig('doctar_stackplot_scaled.pdf',
                      format='pdf',
                      dpi=300,
                      bbox_inches='tight',
                      pad_inches=0.01)
