...
if __name__ ==  "__main__":
    ...
    test_depth = 5000.
    note = 'noise_scaled_z%skm_mwm0.4'%(int(test_depth)/1000)
    false_store_id = None#'false_castor4'
    false_magnitude = -0.4
    do_scale = True 
    ...
    phase_ids_start = ['p','P', 'Pv35p', 'Pv20p',  'begin']
    ...
    print 'SET EVENT DEPTH to %s km'%test_depth
    ...
    print 'CHECK DEPTH IS CORRECT, %s km'%_ref_source.depth
    ...
    test_case_setup.depths = num.arange(800, 10200, 200)
    print test_case_setup.depths
    if check_locations:
    ...
    for i in range(num_tests):
        ...
        if false_store_id:
            for t in test_case_setup.targets:
                t.store_id = store_id 
        if false_magnitude:
            _ref_source.magnitude += false_magnitude
