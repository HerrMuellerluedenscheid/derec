...
if __name__ ==  "__main__":
    ...
    if not socket.gethostname() in ('Mariuss-MacBook.local', 'thunder8'):
        store_dirs.append(pjoin('/',
                                'scratch', 
                                'local1', 
                                'marius'))
    ...
    #note = 'noise_scaled_falsecrust'
    test_depth = 5000.

    note = 'noise_scaled_z%skm'%(int(test_depth/1000))
    ...
    print targets[0].codes[-1]
    targets = filter(lambda x: x.codes[-1]=='Z', targets)
    print targets


    event = model.Event(load=pjoin(data_base_dir, event_file))
    event.depth = test_depth 
    ...
    print ' test depth is %10.12f' %test_depth
    print type(test_depth)
    print ' TEST DEPTH IS EVENT DEPTH :', _ref_source.depth == test_depth
    print ' ref source depth is %10.12f' %_ref_source.depth
    print type(_ref_source.depth)
    ...
    #depths = du.drange(800, 10000, 200)
    depths = du.drange(1000, 9200, 400)
    print 'depths: ', depths
    ...
    z, p, k = butter(4, [0.6*num.pi*2, 4.5*num.pi*2.], 
                       'band', 
                       analog=True, 
                       output='zpk')
    ...
    for i in range(num_tests):
        ...
        #        1,
        #        func='uniform')[0]
        test_source = du.clone(_ref_source)
        test_source.strike = 60
        test_source.dip = 60
        test_source.rake = 60
        location_test_sources = du.test_event_generator(test_source, depths)
        #import pdb 
        #pdb.set_trace
        i+=1
        ...
        if debug:
            ...
            op.stack_plot()
            plt.gcf().savefig('unscald_vertical.pdf', bbox_inches='tight',
                              pad_inches=0.05)
            plt.figure()
            op.stack_plot(scaling=test_case.scaling, force_update=True)
            plt.gcf().savefig('scald_vertical.pdf', bbox_inches='tight',
                              pad_inches=0.05)
            ...
            #               lw=0)
            plt.figure()
            optics.plot_misfit_dict(test_case.misfits, test_case.scaled_misfits,
                                   scaling=test_case.scaling)
            plt.gcf().savefig('misfits.pdf', bbox_inches='tight',
                              pad_inches=0.05)
            plt.show()
