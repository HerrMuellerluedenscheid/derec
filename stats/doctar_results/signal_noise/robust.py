...
if __name__ ==  "__main__":
    ...
    def do_run(nscale):
        file_name = 'robust_check_nscale%s_%s.txt'%(nscale,note)
        noise_scale = nscale
        if test_type=='doctar':
            stf = [[0.,0.15], [0.,1.]]
        ...
        if randomize_mt:
            sc_mom = event.moment_tensor.scalar_moment()
            event.moment_tensor = moment_tensor.MomentTensor.random_dc(scalar_moment=sc_mom)

        _ref_source = DCSource.from_pyrocko_event(event)
        if debug: print 'reference source  magnitude: ', _ref_source.magnitude
        ...
        if false_magnitude:
            ...
            if verbose: print 'setting false magnitude to ', smaller_magnitude_source.magnitude
        ...
        for location_test_sources in location_test_sources_lists:
            ...
            reference_seismograms, snr_sig, snr_sig_p = core.make_reference_trace(
                                                    _ref_source,
                                                    targets,
                                                    engine,
                                                    stf,
            ...
            try:
                ...
                if verbose: print 'both sourcess the same'
                angle_diff = 0
                lateral_shift = 0
            ...
            results.append([lateral_shift, angle_diff, best_misfit,
                best_source.depth, snr, sig, snrp, sigp])
